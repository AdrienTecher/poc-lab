...
### Phase 4 — DONE: PWA + offline, reduced motion, first-run
Installable, plays with the network off, genuinely still when motion is turned down,
and the first run is held by a spec. Three notes for whoever touches them:
  * The service worker is generated by a vite plugin from `sw.template.js`. Do not
    name its `__VERSION__` token in a comment — a first-match replace substitutes the
    comment. And the cache name hashes the public files' CONTENT, not just their
    names, or a changed icon is served from the old cache forever.
  * Reduced motion switches transitions off by PROPERTY. Do not shorten them:
    `transition-property` defaults to `all`, so a duration on `*` gives every element
    a transition it never had, and the rig rewrites transform/cx/cy sixty times a
    second.
